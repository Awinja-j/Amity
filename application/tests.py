...
class TestAmity(unittest.TestCase):
    ...
    def test_create_room_livingspace(self):
        ...
        self.assertTrue(len(self.amity.all_livingspace),1)
    ...
    def test_create_room_wrong_room_type(self):
        ...
        self.assertTrue('his room type is invalid!!')
    def test_add_person(self):
       ...
       self.assertEqual(self.amity.add_person("John Liboyi", "Fellow", "N"),"john liboyi has succesfully been added to Amity. but will be allocated a room one becomes available")

       ''' this tests succesfull creation of Staff who does not want accomodation'''
       self.assertEqual(self.amity.add_person("Mike Katiechi", "Staff", "N"),"mike katiechi has succesfully been added to Amity but will be allocated a room one becomes available")
    ...
    def test_add_person_more_than_once(self):
       ...
       self.assertEqual(self.amity.add_person("James Nyakenyanya","Fellow","N"),"james nyakenyanya has succesfully been added to Amity. but will be allocated a room one becomes available")
    ...
    def test_add_person_existing_swapped_details(self):
       ...
       self.assertEqual(self.amity.add_person("Jeffery Eyama","Fellow","N"),"jeffery eyama has succesfully been added to Amity. but will be allocated a room one becomes available")

    def test_find_userid(self):
        self.amity.create_room('office', ['lade'])
        self.amity.add_person("Jeffery Eyama", "Staff", "N")
        print(self.amity.all_people)
        one = None
    ...
    def test_reallocate(self):
       ...
       for person in self.amity.all_people:
           one = person.person_ID
       self.amity.reallocate_person(one,(self.amity.create_room('office', ['Penny'])))
       self.assertTrue('{} has been reallocated to {} succesfully!!'.format('joan awinja', 'penny'))
    ...
    def test_reallocate_staff_to_livingspace(self):
        ...
        for person in self.amity.all_people:
            one = person.person_ID
        self.amity.reallocate_person(one, self.amity.create_room('livingspace', ['blue']))
        self.assertTrue('Staff cannot be allocated to a livingspace')
    ...
    def test_reallocate_a_person_that_does_not_exist(self):
       '''this tests application response to trying to reallocate a preson who is not in the system'''
    ...
    def test_print_allocations_display(self):
        ...
        self.assertTrue('Offices {} is occupied by: '.format('naija'))
